...
from utils.train_utils_flextok import (
    ...
    train_one_epoch)
from utils.train_utils_policy import train_one_epoch_policy, create_model_and_loss_module
import warnings
...
def main():
    ...
    logger = setup_logger(name="FlexRep-Tok-stage1", log_level="INFO",
    ...
    model, ema_model, loss_module = create_model_and_loss_module(
        config, logger, accelerator, model_type=config.model.vq_model.model_type)
    
    # model.load_state_dict(torch.load("/mnt/petrelfs/luxiaocheng/project/Generation/EfficientGen/FlexGen/pretrained_models/tokenizer_titok_s128.bin"), strict=True)
    # Use compile to accelerate training
    ...
    if config.model.vq_model.finetune_decoder:
        model, loss_module, optimizer, discriminator_optimizer, lr_scheduler, discriminator_lr_scheduler, train_dataloader, val_dataloader = accelerator.prepare(
            model, loss_module, optimizer, discriminator_optimizer, lr_scheduler, discriminator_lr_scheduler, train_dataloader, val_dataloader
        )
    else:
    ...
    for current_epoch in range(first_epoch, num_train_epochs):
        ...
        use_policynet = config.model.vq_model.get("use_policynet", False)
        if use_policynet:
