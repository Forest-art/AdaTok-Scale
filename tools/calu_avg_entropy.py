...
def get_infordensity(x, patch_size=16):
    ...
    return np.clip(density_map.mean(), 0, 200) // 20


# 主函数
def classify_images_by_density(image_dir, patch_size=16):
    import cv2
    entropys = 0
    # 遍历图片目录
    for filename in os.listdir(image_dir)[:1000]:
...
if __name__ == '__main__':
    ...
    classify_images_by_density(image_dir)