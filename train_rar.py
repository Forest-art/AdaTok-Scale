...
from utils.demo_util import get_titok_tokenizer
...
def main():
    ...
    #     hf_hub_download(repo_id="fun-research/TiTok", filename=f"{config.model.vq_model.pretrained_tokenizer_weight}", local_dir="./")
        # hf_hub_download(repo_id="yucornetto/RAR", filename="maskgitvq.jsonl", local_dir="/mnt/hwfile/ai4earth/luxiaocheng/project/Generation/EfficientGen/FlexGen/")
    ...
    # weight = torch.load("rar_l.bin", map_location='cpu')
    # keys_to_remove = ["embeddings.weight", "lm_head.weight", "lm_head.bias"]  # 需要忽略的 key
    ...
    # model.load_state_dict(torch.load("rar_l.bin", map_location="cpu"), strict=True)
