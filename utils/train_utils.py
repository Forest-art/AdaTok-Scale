...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if model_type == "one-d-piece":
        ...
        model_cls = AdapTok
        loss_cls = AdaptiveLoss_Stage2 if config.model.vq_model.finetune_decoder else AdaptiveLoss_Stage1  
...
def train_one_epoch(config, logger, accelerator,
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate([model, loss_module]):
            if model_type == "titok":
                reconstructed_images, extra_results_dict, key_padding_mask, log_prob, entropy_loss, keep_lengths = model(images, clip_image=clip_image)
                reconstructed_images_128 = model(images, token_length=256, clip_image=clip_image)[0]
...
def train_one_epoch_generator(
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            tokenizer.eval()
            outputs = tokenizer.encode(images, clip_image=clip_image)
            input_tokens = outputs[1]["min_encoding_indices"].reshape(images.shape[0], -1)
            keep_length = 256 - outputs[2].sum(dim=1)
            mask = torch.arange(input_tokens.size(1), device=input_tokens.device).unsqueeze(0) > keep_length.unsqueeze(1)
            input_tokens[mask] = 4096
        ...
        with accelerator.accumulate([model]):
            if model_type == 'llamagen':
                shift_logits, loss = model(cond_idx=conditions, idx=input_tokens[:,:-1], targets=input_tokens)
                correct_tokens = (torch.argmax(shift_logits, dim=2) == input_tokens).sum(dim=1) / input_tokens.size(1)
                loss_dict = {"loss": loss, "correct_tokens": correct_tokens.mean()}
            elif model_type == "maskgit":
...
def eval_reconstruction(
    ...
    total_rMSE_baseline = 0
    total_hit_baseline = 0
    total_hit = 0
    total_rMSE = 0
    model.eval()
    fid_metric = FrechetInceptionDistance(feature=2048).to(accelerator.device)
    fid_metric_mean = FrechetInceptionDistance(feature=2048).to(accelerator.device)
    ...
    for batch in progress_bar:  # 使用 tqdm 包装 eval_loader
        ...
        reconstructed_images, model_dict, token_mask = model(images)[:3]
        if pretrained_tokenizer is not None:
            reconstructed_images = pretrained_tokenizer.decode(reconstructed_images.argmax(1))
        reconstructed_images = torch.clamp(reconstructed_images, 0.0, 1.0)
        rMSE = F.mse_loss(reconstructed_images, original_images, reduction='none')
        ...
        total_rMSE += rMSE.mean().item()

        total_hit += (rMSE.mean(dim=list(range(1, 4))) < 0.02).sum().item()

        #compute mean tokens
        mean_tokens = 256 - token_mask.sum().item() / batch[0].shape[0]
        original_images = torch.clone(images)
        reconstructed_images, model_dict, token_mask = model(images, token_length=128)[:3]
        if pretrained_tokenizer is not None:
            reconstructed_images = pretrained_tokenizer.decode(reconstructed_images.argmax(1))
        reconstructed_images = torch.clamp(reconstructed_images, 0.0, 1.0)
        rMSE = F.mse_loss(reconstructed_images, original_images, reduction='none')
        ...
        # FID 更新：torchmetrics 会自动处理特征提取和计算
        fid_metric_mean.update(original_images, real=True)  # 更新真实图像
        fid_metric_mean.update(reconstructed_images.squeeze(2), real=False)  # 更新生成图像

        total_tokens_baseline += token_mask.sum().item()
        total_rMSE_baseline += rMSE.mean().item()
        total_hit_baseline += (rMSE.mean(dim=list(range(1, 4))) < 0.02).sum().item()

    eval_results = dict(
        rFID = fid_metric.compute().item(),
        mean_tokens = 256 - total_tokens / (len(eval_loader) * batch[0].shape[0]),
        mean_rMSE = total_rMSE / len(eval_loader),
        total_hit = total_hit,
    )

    eval_results_mean = dict(
        rFID = fid_metric_mean.compute().item(),
        mean_tokens = 256 - total_tokens_baseline / (len(eval_loader) * batch[0].shape[0]),
        mean_rMSE = total_rMSE_baseline / len(eval_loader),
        total_hit_baseline = total_hit_baseline,
    )

    model.train()

    print(f"flexible eval results: {eval_results} and baseline results: {eval_results_mean}")
...
def reconstruct_images(model, original_images, fnames, accelerator, 
    ...
    reconstructed_images, _ = accelerator.unwrap_model(model)(original_images)
    if pretrained_tokenizer is not None:
        reconstructed_images = pretrained_tokenizer.decode(reconstructed_images.argmax(1))
