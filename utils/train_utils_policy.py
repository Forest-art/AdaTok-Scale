...
def train_one_epoch_policy(config, logger, accelerator,
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        clip_image = batch[-1].to(accelerator.device, memory_format=torch.contiguous_format, non_blocking=True)
...
def train_one_epoch_generator(
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            ...
            # else:
            _, result_dict, _, _, _, keep_lengths = tokenizer.encode(images, token_length=None, clip_image=clip_image)
            input_tokens = result_dict["min_encoding_indices"].reshape(images.shape[0], -1)
        ...
        with accelerator.accumulate([model]):
            if model_type == "maskgit":
                ...
                logits, labels = model(input_tokens, condition, return_labels=True, keep_lengths=None)
...
def eval_reconstruction(
    ...
    for batch in progress_bar:  # 使用 tqdm 包装 eval_loader
        ...
        reconstructed_images, model_dict, _, _ = model(images, entropy_level=batch[2])
        if pretrained_tokenizer is not None:
