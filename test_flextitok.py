...
def main(args):
    ...
    model = FlexTiTok(config)
    # checkpoint_path = config.evaluate.checkpoint_path
    # checkpoint = torch.load(checkpoint_path, map_location="cpu")
    model.load_state_dict(torch.load("./exp/flexgen_s_256_stage1_randmask_1000k_dim64/checkpoint-450000/ema_model/pytorch_model.bin"), strict=True)
    test(config, model)
