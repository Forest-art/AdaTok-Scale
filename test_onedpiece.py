...
def eval_reconstruction(
    ...
    for batch in progress_bar:
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):  
            reconstructed_images, _ = accelerator.unwrap_model(model)(images, token_length=256)
...
def main(args):
    ...
    model.load_state_dict(torch.load("./pretrained_models/tokenizer_titok_l32.bin"), strict=True)
    test(config, model)
