...
class Transformer(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.seq_len = 256
        self.freqs_cis = precompute_freqs_cis(self.seq_len, self.config.dim // self.config.n_head, self.config.rope_base, self.cls_token_num)
    ...
    def forward(
        ...
        if idx is not None and cond_idx is not None: # training or naive inference
            cond_embeddings = self.cls_embedding(cond_idx.to(idx.device), train=self.training)[:,:self.cls_token_num]
