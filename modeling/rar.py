...
class EntropyRAR(BaseModel):
    def __init__(self, config):
        ...
        self.embeddings = nn.Embedding(
            target_codebook_size + 1 + condition_num_classes + 1 + 10, embed_dim)
    ...
    def forward_fn(self, input_ids, condition,
        ...
        input_ids = torch.cat([condition.view(condition.shape[0], -1),
                               input_ids.view(input_ids.shape[0], -1),], dim=1)
        embeddings = self.embeddings(input_ids)
        condition_token = embeddings[:, :condition.shape[1]]
        ...
        if not is_sampling:
            ...
            labels = self.shuffle(labels, orders)
            # randomized permutation: during training, we need to shuffle the input_ids's order but not for sampling
            embeddings = torch.cat([embeddings[:, :condition.shape[1]], self.shuffle(embeddings[:, condition.shape[1]:], orders)], dim=1)
        ...
        # add target-aware pos embed
        target_aware_pos_embed = torch.cat([torch.zeros_like(x[:, :prefix-1]), target_aware_pos_embed_postfix, torch.zeros_like(x[:, -1:])], dim=1)
        x = x + target_aware_pos_embed[:, :x.shape[1]]
        ...
        # seperate condition token for each step, at generation, we start from 1 to seq len
        condition_token = condition_token[:, :1] + self.timesteps_embeddings[:, :x.shape[1]]
        condition_token = condition_token[:, 1:2] + self.timesteps_embeddings[:, :x.shape[1]]
