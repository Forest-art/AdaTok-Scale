...
class TiTokEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.image_size = config.dataset.train.params.image_size 
...
class TiTokDecoder(nn.Module):
    ...
    def forward(self, z_quantized, key_padding_mask=None):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        for i in range(self.num_layers):
            x = self.transformer[i](x, key_padding_mask)
        x = x.permute(1, 0, 2)  # LND -> NLD
...
class EntropyTiTokEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.image_size = config.dataset.train.params.image_size 
        ...
        self.ln_post = nn.LayerNorm(self.width)
        self.conv_out = nn.Conv2d(self.width, self.token_size, kernel_size=1, bias=True)
        self.entropy_level_embedding = nn.Embedding(11, self.width)


    def forward(self, pixel_values, latent_tokens, entropy_level):
        entropy_level_embedding = self.entropy_level_embedding(entropy_level.to(torch.int))
        ...
        x = torch.cat([_expand_token(self.class_embedding, x.shape[0]).to(x.dtype), x], dim=1)
        x = x + self.positional_embedding.to(x.dtype) # shape = [*, grid ** 2 + 1, width]
        x = torch.cat([x, entropy_level_embedding.unsqueeze(1)], dim=1) # shape = [*, grid ** 2 + 2, width]
        ...
        latent_tokens = x[:, 2+self.grid_size**2:]
        latent_tokens = self.ln_post(latent_tokens)
        ...
        latent_tokens = self.conv_out(latent_tokens)
        latent_tokens = latent_tokens.reshape(batch_size, self.token_size, 1, self.num_latent_tokens)
        return latent_tokens, feat


class EntropyTiTokDecoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.image_size = config.dataset.train.params.image_size 
    ...
    def forward(self, z_quantized, attn_mask=None, entropy_level = None):
        N, C, H, W = z_quantized.shape
        # assert H == 1 and W == self.num_latent_tokens, f"{H}, {W}, {self.num_latent_tokens}"
        x = z_quantized.reshape(N, C*H, W).permute(0, 2, 1) # NLD
        x = self.decoder_embed(x)
        ...
        mask_tokens = self.mask_token.repeat(batchsize, self.grid_size**2, 1).to(x.dtype)
        mask_tokens = torch.cat([_expand_token(self.class_embedding, mask_tokens.shape[0]).to(mask_tokens.dtype), mask_tokens], dim=1)
        mask_tokens = mask_tokens + self.positional_embedding.to(mask_tokens.dtype)

        x = x + self.latent_token_positional_embedding[:seq_len]
        if entropy_level is not None:
            entropy_level_embedding = self.entropy_level_embedding(entropy_level.to(torch.int))
            x = torch.cat([entropy_level_embedding.unsqueeze(1), x], dim=1)
        x = torch.cat([mask_tokens, x], dim=1)
