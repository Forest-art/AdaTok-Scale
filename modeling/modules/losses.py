...
class ReconstructionLoss_Stage1(torch.nn.Module):
    ...
    def forward(self,
        ...
        return self._forward_generator(target_codes, reconstructions, quantizer_loss, dino_feat)
    ...
    def _forward_generator(self,
        ...
        if dino_feat is not None:
            vf_loss = 10 * F.mse_loss(dino_feat.squeeze().reshape(dino_feat.shape[0], dino_feat.shape[1], -1).mean(dim=[-1]), quantizer_loss['z_quantized'].squeeze().mean(dim=-1))

        total_loss = reconstruction_loss + \
            self.quantizer_weight * quantizer_loss["quantizer_loss"] + vf_loss
...
class AdaptiveLoss_Stage1(torch.nn.Module):
    def __init__(
        self,
        config
    ):
    ...
    def forward(self,
        ...
        return self._forward_generator(target_codes, reconstructions, quantizer_loss, log_prob, entropy_loss, keep_lengths, current_step)

    # 每个训练步骤更新熵权重
    def get_entropy_weight(self, current_step):
        # 指数衰减
        initial_entropy_weight = 0.1  # 初始权重较大，鼓励探索
        final_entropy_weight = 0.01  # 最终权重较小，避免干扰
        decay_steps = 10000           # 衰减步数（根据训练总步数调整）
        entropy_weight = initial_entropy_weight * (final_entropy_weight / initial_entropy_weight) ** (current_step / decay_steps)
        # 或线性衰减
        # entropy_weight = initial_entropy_weight - (initial_entropy_weight - final_entropy_weight) * (current_step / decay_steps)
        return max(entropy_weight, final_entropy_weight)  # 确保不低于最终值
    ...
    def _forward_generator(self,
        ...
        batch_size = reconstructions.shape[0]
        reconstruction_loss = loss_fct(reconstructions.view(batch_size, self.target_codebook_size, -1), target_codes.view(batch_size, -1)).mean(dim=-1)

        # reward = -reconstruction_loss.detach() + 1.0 * torch.log(1 - keep_lengths.float() / 256 + 1e-8)
        token_reward = torch.where(
            keep_lengths < 150,
            2.0 * (keep_lengths.float() - 150) / 256,  # 斜率为 5
            0 * (keep_lengths.float() - 150) / 256  # 斜率为 1/256
        )
        reward = -reconstruction_loss.detach() - token_reward
        # 改进：归一化奖励（按 batch 动态缩放）
        baseline = reward.mean()
        policy_loss = -((reward - baseline) * log_prob).mean()

        entropy_weight = self.get_entropy_weight(current_step)
        ...
        loss_dict = dict(
            ...
            commitment_loss=quantizer_loss["commitment_loss"].detach(),
            codebook_loss=quantizer_loss["codebook_loss"].detach(),
            keep_lengths=keep_lengths.float().mean(),
...
class AdaptiveLoss_Stage2(torch.nn.Module):
    def __init__(
        self,
        config
    ):
        ...
        loss_config = config.losses
        self.quantizer_weight = loss_config.quantizer_weight
        self.rec_baseline = loss_config.rec_baseline
    ...
    def forward(self,
                ...
                keep_lengths,
                current_step,
                reconstructed_images_128
                ) -> Tuple[torch.Tensor, Mapping[Text, torch.Tensor]]:
        return self._forward_generator(target_codes, reconstructions, quantizer_loss, log_prob, entropy_loss, keep_lengths, current_step, reconstructed_images_128)
    ...
    def batch_calculate_reward(self, token_lengths, rec_losses):
        # 类型转换确保计算稳定性
        tl = token_lengths.float()
        rl = rec_losses.float()
        
        # 核心条件判断（向量化）
        is_satisfied = (rl < self.rec_baseline)
        ...
        reward = torch.where(
            ...
            - (rl - self.rec_baseline) * 10 + tl/255  # 综合惩罚
        )
        return reward
    ...
    def _forward_generator(self,
        ...
        reconstructions = reconstructions.contiguous()
        batch_size = reconstructions.shape[0]
        reconstruction_loss = F.mse_loss(reconstructions, target_codes, reduction='none').mean(dim=list(range(1, 4)))
        reconstruction_loss_128 = F.mse_loss(reconstructed_images_128, target_codes, reduction='none').mean(dim=list(range(1, 4)))

        # reward = -reconstruction_loss.detach() + 1.0 * torch.log(1 - keep_lengths.float() / 256 + 1e-8)
        reward = self.batch_calculate_reward(keep_lengths, reconstruction_loss)
        batch_hit_count = (reconstruction_loss < self.rec_baseline).int()
        # reward = -10 * reconstruction_loss.detach() - token_reward
        # reward = 20 * batch_hit_reward - 1 * token_reward
        
        reward_baseline = self.batch_calculate_reward(torch.clamp(keep_lengths, 255, 255), reconstruction_loss_128)
        # 改进：归一化奖励（按 batch 动态缩放）
        # baseline = reward.mean()
        ...
        loss_dict = dict(
            ...
            entropy_loss=entropy_loss.detach(),
            log_prob=log_prob.mean().detach(),
            batch_hit_count=batch_hit_count.float().sum(),
            batch_hit_count_baseline = ((reconstruction_loss_128 < 0.02).float()).sum(),
            reward_baseline=reward_baseline, 
            reconstruction_loss_128=reconstruction_loss_128.mean(),
        )
