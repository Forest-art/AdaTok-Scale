...
from modeling.modules.blocks import TiTokEncoder, TiTokDecoder, EntropyTiTokEncoder, EntropyTiTokDecoder
...
class EntropyFlexTiTok(BaseModel, PyTorchModelHubMixin, tags=["arxiv:2406.07550", "image-tokenization"], repo_url="https://github.com/bytedance/1d-tokenizer", license="apache-2.0"):
    def __init__(self, config):
        ...
        self.encoder = EntropyTiTokEncoder(config)
        self.decoder = EntropyTiTokDecoder(config)
    ...
    def get_key_padding_mask(self, mask_length_start=257, mask_length_end=513, batch_size=1):
        ...
        indices = torch.arange(seq_length)  # 创建一个从 0 到 seq_length-1 的索引张量
        key_padding_mask = indices >= mask_lengths.unsqueeze(1)  # 广播比较，生成 mask

        return key_padding_mask


    def encode(self, x, entropy_level):
        import random
        if self.finetune_decoder:
            with torch.no_grad():
                self.encoder.eval()
                self.quantize.eval()
                z = self.encoder(pixel_values=x, latent_tokens=self.latent_tokens, entropy_level=entropy_level)
                attn_mask, token_mask = self.gen_attn_mask(x)
                if self.training:
                    z = z * token_mask.to(z.device)
            ...
            z = self.encoder(pixel_values=x, latent_tokens=self.latent_tokens, entropy_level=entropy_level)
            key_padding_mask = self.get_key_padding_mask(257, 513, z.shape[0])
            if self.quantize_mode == "vq":
                z_quantized, result_dict = self.quantize(z)
            elif self.quantize_mode == "vae":
    ...
    def decode(self, z_quantized, entropy_level=None):
        key_padding_mask = self.get_key_padding_mask(258, 514, z_quantized.shape[0]).to(z_quantized.device)
        if self.training:
            decoded = self.decoder(z_quantized, attn_mask=key_padding_mask, entropy_level=entropy_level)
        else:
            decoded = self.decoder(z_quantized, attn_mask=None, entropy_level=entropy_level)
        if self.finetune_decoder:
    ...
    def forward(self, x, entropy_level=None):
        z_quantized, result_dict = self.encode(x, entropy_level)
        decoded = self.decode(z_quantized, entropy_level=entropy_level)
