...
class FlexTiTok(BaseModel, PyTorchModelHubMixin, tags=["arxiv:2406.07550", "image-tokenization"], repo_url="https://github.com/bytedance/1d-tokenizer", license="apache-2.0"):
    ...
    def encode(self, x, token_length=None):
        import random
        if self.finetune_decoder:
            with torch.no_grad():
                self.encoder.eval()
                ...
                mask_lengths = torch.randint(258, 257+256, (z.shape[0],))
                key_padding_mask = torch.zeros(z.shape[0], 257+256, dtype=torch.bool).to(z.device)
                seq_length = key_padding_mask.size(1)  # 获取序列长度
            ...
            z = self.encoder(pixel_values=x, latent_tokens=self.latent_tokens)
            # attn_mask, token_mask = self.gen_attn_mask(x)
            keep_lengths = torch.randint(1, 256, (z.shape[0],))
    ...
    def forward(self, x, token_length=None):
